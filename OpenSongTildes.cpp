...
{
    std::ifstream ifs(oldname.c_str(), std::ios::binary);
    std::ofstream ofs(newname.c_str(), std::ios::binary);
    ofs << ifs.rdbuf();
}
...
int main()
{
    ...
    while ((dirp = readdir( dp )))
    {
        ...
        while( i < 256 && dirp->d_name[ i ] != 0 )
        {
            ...
            if ( dictionary.find( letter[ 0 ] ) != dictionary.end() ) // has tilde?
            {
        ...
        if ( mod )
        {
            ...
            // error 3: path not found: does not work with UNIX routes???
